...
<script>
	...
	$(document).ready(function() {
    	...
    	<?php for ($i=0; $i < count($ledgers) ; $i++) { 
    			echo "ledger_data.push('".$ledgers[$i]->iextetacl_name."');";
    	} ?>
    	$( "#j_from" ).autocomplete({
            source: ledger_data
        });
		...
		$('#submit').click(function(e) {
		    ...
		    if(selected_je != 0) {
    		    ...
    		    $.post('<?php echo base_url()."Accounting/record_journal_entry/".$code; ?>', {
    		        'r' : records,
    		        'selected' : 'true'
    		    }, function(d,s,x) {
		...
		function redirect(ref) {
			if (ref=="j") {
				...
				window.location = "<?php echo base_url().'Accounting/ledger_details/'.$code.'/'.$refid; ?>";
			} else if(ref=="g") {
				window.location = "<?php echo base_url().'Accounting/group_details/'.$code.'/'.$refid; ?>";
			} else if(ref=="tr") {
				window.location = "<?php echo base_url().'Accounting/trial_balance/'.$code.'/'.$refid; ?>";
			}
