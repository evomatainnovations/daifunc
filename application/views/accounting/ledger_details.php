    ...
    <div class="mdl-grid">
        <div class="mdl-cell mdl-cell--12-col">
	        <button class="mdl-button mdl-js-button mdl-button--colored" id="add_entry"><i class="material-icons">add</i></button>    <?php if(isset($details)) {
	            if($details[0]->iextetacl_starred == 0) {
...
<script>
	...
	$(document).ready(function() {
		...
		$('#filter_records').click(function(e) {
		    e.preventDefault();
		    $.post('<?php echo base_url()."Accounting/filter_journal_entries/".$code; ?>', {
		        'f_dt' : $('#t_f_date').val(),
		        't_dt' : $('#t_t_date').val(),
		        'f' : $('#t_from').val(),
		        't' : $('#t_to').val(),
	    ...
	    $('#details').on('click','.edit', function(e) {
            e.preventDefault();
            window.location = "<?php echo base_url().'Accounting/journal_entry_details/l/'.$code.'/'; ?>" + $(this).prop('id') + "<?php echo '/'.$lid; ?>";
        });
        
        $('#details').on('click','.delete', function(e) {
            e.preventDefault();
            $(this).attr('disabled','disabled');
            $.post('<?php echo base_url()."Accounting/delete_journal_entry/".$code.'/'; ?>' + $(this).prop('id') + '<?php echo "/l/".$lid; ?>', {}, function(d,s,x) {
                var b = JSON.parse(d);
                fill_main_table(b);
            }, "text"); 
        });
		...
		$('#add_entry').click(function(e) {
            e.preventDefault();
            window.location = "<?php echo base_url().'Accounting/journal_entry_details/l/'.$code.'/null/'.$lid; ?>";
        });

        $('#view_details').click(function(e) {
            e.preventDefault();
            window.location = "<?php echo base_url().'Accounting/edit_ledger/'.$code.'/'.$lid; ?>";
        });

        $('#starred').click(function(e) {
	        e.preventDefault();
	        $.post('<?php echo base_url()."Accounting/update_ledger_star/".$code; ?>', {
	            'l' : <?php echo $lid; ?>,
	            's' : 1
	        }, function(d,s,x) {
	            window.location = "<?php echo base_url().'Accounting/ledger_details/'.$code.'/'.$lid; ?>";
	        })
	    })
	    ...
	    $('#download').click(function(e) {
		    ...
		    $('.purchase_table').tableExport({
                ...
                filename: '<?php if (isset($details)) { echo $details['0']->iextetacl_name; } else { echo 'Ledger'; } ?>' + ' - ' + s_dt,
