	        ...
	        <div class="mdl-card mdl-shadow--4dp">
                ...
                <div class="mdl-card__actions mdl-card--border" style="text-align:left;">
                    <?php for($i=0;$i<count($ledgers);$i++) {
            	        echo '<button style="width:100%; text-align:left;" class="mdl-button mdl-js-button ledger_specific" id="'.$ledgers[$i]->iextetacl_id.'"><i class="material-icons">star_border</i> '.$ledgers[$i]->iextetacl_name.'</button><hr>';
            	    } ?>
                </div>
	        </div>
	    </div>
...
<script>
	$(document).ready(function() {
	    $('#journal_entry').click(function(e) {
	        e.preventDefault();
	        window.location = "<?php echo base_url().'/Accounting/journal_entries/'.$code; ?>";
	    })
	    
	    $('#ledgers').click(function(e) {
	        e.preventDefault();
	        window.location = "<?php echo base_url().'/Accounting/ledgers/'.$code; ?>";
	    });
	    
	    $('.ledger_specific').click(function(e) {
	        e.preventDefault();
	        window.location = "<?php echo base_url().'/Accounting/ledger_details/'; ?>" + $(this).prop('id');
	    });
	    
	    $('#groups').click(function(e) {
	        e.preventDefault();
	        window.location = "<?php echo base_url().'/Accounting/groups/'.$code; ?>";
	    });
	    
	    $('#classes').click(function(e) {
	        e.preventDefault();
	        window.location = "<?php echo base_url().'/Accounting/classes/'.$code; ?>";
	    });
	    
	    $('#trial_balance').click(function(e) {
	        e.preventDefault();
	        window.location = "<?php echo base_url().'/Accounting/trial_balance'; ?>";
	    });
	    
	    $('#receive_payments').click(function(e) {
	        e.preventDefault();
	        window.location = "<?php echo base_url().'/Accounting/vouchers/receipt'; ?>";
	    });
	    
	    $('#issue_payments').click(function(e) {
	        e.preventDefault();
	        window.location = "<?php echo base_url().'/Accounting/vouchers/payment'; ?>";
	    });
