...
<script>
	...
	$(document).ready(function() {
	    ...
	    $('#add_entry').click(function(e) {
	        e.preventDefault();
	        window.location = "<?php echo base_url().'Accounting/journal_entry_details/g/null/'.$code.'/'.$gid; ?>";
	    });

	    $('#view_details').click(function(e) {
            e.preventDefault();
            window.location = "<?php echo base_url().'Accounting/edit_group/'.$code.'/'.$gid; ?>";
        });
	    
	    $('#details').on('click','.edit', function(e) {
            e.preventDefault();
            window.location = "<?php echo base_url().'Accounting/journal_entry_details/g/'.$code.'/'; ?>" + $(this).prop('id') + "<?php echo '/'.$gid; ?>";
        });
        
        $('#details').on('click','.delete', function(e) {
            e.preventDefault();
            $(this).attr('disabled','disabled');
            $.post('<?php echo base_url()."Accounting/delete_journal_entry/".$code.'/'; ?>' + $(this).prop('id') + '<?php echo "/g/".$gid; ?>', {}, function(d,s,x) {
                var b = JSON.parse(d);
                fill_main_table(b);
            }, "text"); 
        });
