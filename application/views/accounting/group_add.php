			...
			<div class="mdl-card mdl-shadow--4dp">
				...
				<div class="mdl-card__supporting-text" style="width: auto;">
				    <div class="mdl-textfield mdl-js-textfield mdl-textfield--floating-label">
						<input type="text" id="vendors" class="mdl-textfield__input" value="<?php if(isset($detail)) echo $detail[0]->iextetacg_name; ?>">
...
<script>
    $(document).ready(function(e) {
        ...
        var tag_data = [];
		<?php for($i=0;$i<count($groups);$i++) { echo 'tag_data.push("'.$groups[$i]->iextetacg_name.'");'; } ?>
        $( "#mygroups" ).autocomplete({
            ...
            select : function(e, u) {
                $.post('<?php echo base_url()."/Accounting/fetch_group_classes/".$code; ?>', {
                    'g': u.item.value
                }, function(d,s,x) {
                    $('#myclasses').val(d);
                })
		...
		var tag_data2 = [];
		<?php for($i=0;$i<count($classes);$i++) { echo 'tag_data2.push("'.$classes[$i]->iextetacc_name.'");'; } ?>
		...
		$('#del').click(function(e) {
    		e.preventDefault();
    		window.location = "<?php if (isset($detail)) echo base_url().'/Accounting/delete_group/'.$code.'/'.$gid; ?>";
    	});
        ...
        $('#submit').click(function(e) {
            ...
            $.post('<?php if(isset($detail)) { echo base_url()."/Accounting/save_group/".$code.'/'.$gid; } else { echo base_url()."/Accounting/save_group/".$code; } ?>', {
                'name': $('#vendors').val(),
