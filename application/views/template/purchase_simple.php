	...
	for($i=0;$i < count($temp_copies);$i++){ 
		...
		$a.= '<tr><td>'.$basic[0]->ic_name.'</td></tr>';
			for($j=0;$j < count($property); $j++){
				$a.='<tr><td>'.$property[$j]->iexteprpt_property_value.'</td></tr>';
			}
		$a.='</table>';
		...
		$a.='<table style="width: 100%;text-align: center;"><thead><tr><th>Sr. No.</th><th>Particulars</th><th>Rate</th><th>Qty</th><th>Tax</th><th>Disc.</th><th style="text-align: right;">Amount</th></tr></thead><tbody>';
			for($k=0;$k< count($details);$k++){
				...
				$a.='<br>'.$details[$k]->iexteppd_serial_number.'</td><td style="padding-top: 2%;">'.$details[$k]->iexteppd_rate.'</td><td style="padding-top: 2%;">'.$details[$k]->iexteppd_qty.'</td><td style="padding-top: 2%;">'.$details[$k]->ittxg_group_name.'</td>';
				$disc = 0;
				if($details[$k]->iexteppd_discount != ''){
					$a.= '<td style="padding-top: 2%;">'.$details[$k]->iexteppd_discount.'</td>';
