					...
					for($j=0;$j<count($temp_copies);$j++)

					 {
				        ...
				        for ($i=0; $i < count($details) ; $i++) {
							...
							$content.='<td class="detail_center" style="text-align:center;">'.$details[$i]->iexteqpd_qty.'</td><td class="detail_right" style="text-align:right;">'.money_format("%.2n", $details[$i]->iexteqpd_rate).'</td>';

							$content.='<td class="detail_right" style="text-align:right;">'.money_format("%.2n", $details[$i]->iexteqpd_amount).'</td>';
							...
							for ($k=0; $k < count($quotation_taxes) ; $k++) { 
								...
								if ($details[$i]->iexteqpd_id == $quotation_taxes[$k]->iexteqpt_d_id) {
								    
								    $tax_total=$tax_total + $details[$k]->iexteqpd_amount;
								}
							}
                            $product_value = $product_value + $details[$i]->iexteqpd_amount;
