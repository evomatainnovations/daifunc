	...
	for($i=0;$i < count($temp_copies);$i++){ 
		...
		$a.= '<tr><td>'.$basic[0]->ic_name.'</td></tr>';
			for($j=0;$j < count($property); $j++){
				$a.='<tr><td>'.$property[$j]->iexteinpt_property_value.'</td></tr>';
			}
		$a.='</table>';
		...
		$a.='</table>';
		$a.='<table style="text-align: left;font-weight: bold;margin-top: 5%;width: 20%;"><tbody><tr><td>Invoice No</td><td>:</td><td>'.$s_txn_id.'</td></tr><tr><td>GST No.</td><td>:</td><td>'.$u_gst.'</td></tr><tr><td>Date</td><td>:</td><td>'.$s_txn_date.'</td></tr></tbody></table>';
		$a.='<hr>';
		$a.='<div>';
		$discount_flg = 'false';
		...
		$a.='<table style="width: 100%;text-align: center;"><thead><tr><th>Sr. No.</th><th>Particulars</th><th>Rate</th><th>Qty</th><th>Tax</th>';
		if ($discount_flg == 'true') {
			$a.= '<th>Disc.</th>';
		}
		$a.= '<th style="text-align: right;">Amount</th></tr></thead><tbody>';
			for($k=0;$k< count($details);$k++){
				$n = $k + 1;
				...
				$a .='<br>'.$details[$k]->iexteinpd_serial_number.'</td><td style="padding-top: 2%;">'.$details[$k]->iexteinpd_rate.'</td><td style="padding-top: 2%;">'.$details[$k]->iexteinpd_qty.'</td><td style="padding-top: 2%;">'.$details[$k]->ittxg_group_name.'</td>';
				$disc = 0;
				if($details[$k]->iexteinpd_discount != ''){
					$a.= '<td style="padding-top: 2%;">'.$details[$k]->iexteinpd_discount.'</td>';
