...
<?php
	for($i=0;$i < count($temp_copies);$i++){ 
		...
		for($j=0;$j < count($property); $j++){
			$a.='<tr><td>'.$property[$j]->iextetorp_property_value.'</td></tr>';
		}
		$a.='</table>';
		...
		$a.='</table>';
		$a.='<table style="text-align: left;font-weight: bold;margin-top: 5%;width: 20%;"><tbody><tr><td>Order No</td><td>:</td><td>'.$s_txn_id.'</td></tr><tr><td>Date</td><td>:</td><td>'.$s_txn_date.'</td></tr></tbody></table>';
		$a.='<hr>';
		$a.='<div>';
		$a.='<table style="width: 100%;text-align: center;"><thead><tr><th>Sr. No.</th><th>Particulars</th><th style="text-align: right;">Qty</th></tr></thead><tbody>';
			for($k=0;$k< count($details);$k++){
				$n = $k + 1;
				$a.= '<tr><td style="padding-top: 2%;">'.$n.'</td><td style="padding-top: 2%;">';
				if ($details[$k]->iextetodp_alias == 'true'){
					$a.= $details[$k]->ipp_alias;
				}else{
					$a.= $details[$k]->ip_product;
				}
				$a .= '</td><td style="padding-top: 2%;">'.$details[$k]->iextetodp_qty.'</td></tr>';
			}
		$a.='</tbody></table></div>';
		$a.='<hr>';
