...
<?php for($i=0;$i < count($temp_copies);$i++){  ?>
	<body>
		...
		<article>
			...
			<?php
				...
				for($j=0;$j < count($property); $j++){
					echo '<p>'.$property[$j]->iexteinvept_property_value.'</p>';
				}
			?>
			</address>
			<table class="meta">
			...
			<table class="inventory">
				...
				<?php	
					for($k=0;$k< count($details);$k++){
						...
						echo '</span></td><td style="text-align: center;"><span>';
						// if ($type == 'inward') {
							echo $details[$k]->iexteid_balance;
						// }else{
							// echo $details[$k]->iexteid_outward;
						// }
						echo '</span></td><td style="text-align: right;"><span>'.$details[$k]->iexteid_serial_number.'</span></td></tr>';
					}
				?>	
				</tbody>
			</table>
		...
		<?php 
			if($s_txn_note != ''){
				echo '<aside><h1><span>Additional Notes</span></h1><hr><div><p>'.$s_txn_note.'</p></div></aside>';
			}
		 ?>
	</body>
<?php
