...
<main class="mdl-layout__content">
	...
	<div class="mdl-grid">
		...
		<div class="mdl-cell mdl-cell--4-col">
			<a href="<?php echo base_url().'Enterprise/document_terms/Purchase_order/'.$code; ?>"><button class="mdl-button mdl-js-button mdl-button--raised" style="width: 100%;">Terms</button></a>
		</div>
		<div class="mdl-cell mdl-cell--4-col"></div>
	</div>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#add').click(function(e) {
			e.preventDefault();
			window.location = "<?php echo base_url().'Purchase_order/invoice_add/'.$mod_id.'/'.$code; ?>";
		});

		$('#details').on('click', '.tbl_view', (function(e) {
			e.preventDefault();
			var tid = $(this).prop('id');
			window.location = "<?php echo base_url().'Purchase_order/invoice_add/'.$mod_id.'/'.$code.'/' ?>"+tid;

		}));

		$('#invoice_setting').click(function (e) {
			e.preventDefault();
			window.location = "<?php echo base_url().'Enterprise/module_setting/Purchase_order/'.$code; ?>";
		});

		$('#fixed-header-drawer-exp').change(function(e) {
			e.preventDefault();
			$.post('<?php echo base_url()."Purchase_order/invoice_search/".$code; ?>', {
				'search' : $(this).val()
		...
		$('#check').click(function(e) {
			e.preventDefault();
			$.post('<?php echo base_url()."Purchase_order/invoice_filter/".$code; ?>', {
				'from' : $('#fr_date').val(),
				'to': $('#to_date').val(),
