	...
	<div class="mdl-grid">
		...
		<?php
			if (isset($edit_customer)) {
				...
				echo '<div class="mdl-cell mdl-cell--2-col" style="text-align: center;">';
				echo '<a href="';
				echo base_url().'Customers/delete_customer/'.$cid;
	...
	$(document).ready(function() {
		...
		$('#submit').click(function(e) {
			...
			<?php if (isset($edit_customer)) {
					echo "$.post('".base_url()."Customers/update_customer/".$cid."', {'name' : customer_name, 'new_property' : c_new_data, 'value' : c_value, 'tags' : customer_info }, function(data, status, xhr) { uploadfiledata(".$cid.");/*window.location = '".base_url()."Customers'*/ }, 'text');";
				} else {
					echo "$.post('".base_url()."Customers/save_customer', {'name' : customer_name, 'new_property' : c_new_data, 'value' : c_value, 'tags' : customer_info }, function(data, status, xhr) { uploadfiledata(data); /*window.location = '".base_url()."Customers'*/}, 'text');";
				}
			?>
...
function uploadfiledata(stid) {
	...
	if($('.upload')[0].files[0]) {
		...
		flnm = "";
		$.ajax({
			url: "<?php echo base_url().'Customers/uploadfile/'; ?>" + stid, // Url to which the request is send
