					...
					<div class="mdl-grid" style="text-align: center; display: inline-flex;">
						<?php
							if(isset($edit_invoice)) {
								...
								echo '<button class="mdl-button mdl-button--accent" style="margin-top : 10px" id="p_ship_label"><i class="material-icons">directions_boat</i> Print shipping label</button>';
...
<div class="modal fade" id="sel_temp" role="dialog">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body" style="text-align: center;">
				<?php echo '<a href="'.base_url().'Enterprise/module_setting/Inventory_new/'.$code.'"><button class="mdl-button mdl-js-button mdl-button--raise mdl-button--colored"> Click to select template</button><a>'; ?>
			</div>
		</div>
	</div>
</div>
...
<script type="text/javascript">
	...
	<?php
		...
		if (isset($edit_invoice)) {
			for ($i=0; $i <count($edit_invoice) ; $i++) {
				// if ($type == 'inward') {
					echo "product_list.push({'id' : ".$i.",'product' : '".$edit_invoice[$i]->ip_product."', 'qty' : '".$edit_invoice[$i]->iin_inward."','sn' : '".$edit_invoice[$i]->iin_serial_number."' , 'alias' : '".$edit_invoice[$i]->iin_alias."' });";
				// }else{
				// 	echo "product_list.push({'id' : ".$i.",'product' : '".$edit_invoice[$i]->ip_product."', 'qty' : '".$edit_invoice[$i]->iin_inward."','sn' : '".$edit_invoice[$i]->iin_serial_number."' , 'alias' : '".$edit_invoice[$i]->iin_alias."' });";
				// }
			}
	...
	$(document).ready( function() {
    	...
    	$('#save_project').click(function (e) {
    		...
    		$.post('<?php if (isset($edit_invoice)) { echo base_url()."Inventory/save_to_project/".$code."/".$tid."/";}?>'+pid
    	...
    	$('.pro_to_self').click(function (e) {
        	e.preventDefault();
        	$.post('<?php if (isset($edit_invoice)) { echo base_url()."Inventory/inventory_transfer/".$code."/".$tid."/0";}?>'
			, function(data, status, xhr) {
				window.location = '<?php echo base_url()."Inventory/inventory_new/".$mod_id."/".$code; ?>';
			}, "text");
        });
		...
		$('#grp_body').on('click','.transfer_to_group',function (e) {
			...
			$.post('<?php if (isset($edit_invoice)) { echo base_url()."Inventory/inventory_transfer/".$code."/".$tid."/";}?>'+gid
			,function (data, status , xhr) {
				window.location = '<?php echo base_url()."Inventory/inventory_new/".$mod_id."/".$code; ?>';
			}, 'text');
		});
		...
		$('#invoice_user_add').click(function (e) {
			...
			$.post('<?php if (isset($edit_invoice)) { echo base_url()."Inventory/inventory_transfer_user/".$code."/".$tid; } ?>', {
				'cust_name' : name[0]
			}, function(data, status, xhr) {
		...
		$('.close_modal').click(function (e) {
     	...
     	$('.doc_file').click(function (e) {
     		e.preventDefault();
     		var id = $(this).prop('id');
			window.location = '<?php echo base_url()."Inventory/upload_doc_download/".$code."/";?>'+id;
     	});
    	...
    	$('#cust_draft').click(function (e) {
    		e.preventDefault();
    		var type = 'draft'
    		$.post('<?php if (isset($edit_invoice)) { echo base_url()."Inventory/update_inventory/".$tid."/".$code."/"; } else {echo base_url()."Inventory/save_inventory/".$code;} ?>'+draft,{
    	...
    	$('#formal_submit').click(function (e) {
    		...
    		$.post('<?php if (isset($edit_invoice)) { echo base_url()."Inventory/update_inventory/".$tid."/".$code."/"; }else{ echo base_url()."Inventory/save_inventory/".$code."/";}?>'+type,{
	    		'customer' : $('#cust_name').val(),
				'txn_no' : $('#i_txn_no').val(),
				'txn_date' : $('#i_txn_date').val(),
				'product' : product_list,
	 	...
	 	$('#cust_print').click(function(e){
	    	e.preventDefault();
	    	$.post('<?php echo base_url()."Inventory/check_template/".$code."/".$mod_id; ?>',{
			}, function(data, status, xhr) {
				if (data == 'true') {
					window.location = "<?php if (isset($edit_invoice)) { echo base_url()."Inventory/inventory_download/p/".$code."/".$mod_id."/".$tid."/".$type;} ?>";
				}else{
					$('#sel_temp').modal('show');
				}
			}, "text");
	    ...
	    $('#cust_download').click(function(e){
	    	...
	    	$.post('<?php echo base_url()."Inventory/check_template/".$code."/".$mod_id; ?>',{
				...
				if (data == 'true') {
					window.location = "<?php if (isset($edit_invoice)) { echo base_url()."Inventory/inventory_download/d/".$code."/".$mod_id."/".$tid."/".$type;} ?>";
				}else{
					$('#sel_temp').modal('show');
				}
			}, "text");
	    ...
	    $('#p_ship_label').click(function(e){
	    	e.preventDefault();
	    	window.location = "<?php if (isset($edit_invoice)) { echo base_url()."Inventory/inventory_ship_label/".$code."/".$mod_id."/".$tid;} ?>";
	    });
		...
		$('#cust_delete').click(function(e){
	    	e.preventDefault();
	    	window.location = "<?php if (isset($edit_invoice)) { echo base_url()."Inventory/delete_inventory/".$code."/".$tid;} ?>" ;
	    });
		...
		$('#send_mail').click(function(e){
			e.preventDefault();
			$('.loader').show();
			$.post('<?php if (isset($edit_invoice)) echo base_url()."Inventory/save_inventory_mail/".$mod_id."/".$tid."/".$code."/"; ?>'+type,
		...
		function get_pro_details(p_name) {
			$.post('<?php echo base_url()."Inventory/add_product_inventory/".$code; ?>',{
				'pname' : p_name
			}, function(data, status, xhr) {
				var a = JSON.parse(data);
    	...
    	function get_details(customer) {
			$.post('<?php echo base_url()."Inventory/cust_details/".$code."/"; ?>',{
				'c' : customer
			}, function(data, status, xhr) {
				var a = JSON.parse(data);
				var out = '';
    	...
    	function upload_files(pid,cid){
    		if($('.proposal_doc')[0].files[0]) {
				...
				$.ajax({
					url: "<?php echo base_url().'Inventory/inventory_doc_upload/'.$code."/".$mod_id."/";?>"+pid+'/'+cid, // Url to which the request is send
					...
					{
						window.location = '<?php echo base_url()."Inventory/inventory_edit/".$type."/".$code."/".$mod_id."/"; ?>'+pid;
					}
				});
			}else{
				window.location = '<?php echo base_url()."Inventory/inventory_edit/".$type."/".$code."/".$mod_id."/"; ?>'+pid;
