	...
	<div class="mdl-grid">
		<div class="mdl-tabs mdl-js-tabs mdl-js-ripple-effect">
            ...
            <div class="mdl-tabs__tab-bar" style="width:100%">
	            <a href="#p_formal" class="mdl-tabs__tab is-active" id="formal" style="color:black">Formal Invoice</a>
...
<div class="modal fade" id="sel_temp" role="dialog">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body" style="text-align: center;">
				<?php echo '<a href="'.base_url().'Enterprise/module_setting/Invoice/'.$code.'"><button class="mdl-button mdl-js-button mdl-button--raise mdl-button--colored"> Click to select template</button><a>'; ?>
			</div>
		</div>
	</div>
</div>
	...
	?>
	 $(document).ready( function() {
		...
		$('#grp_body').on('click','.transfer_to_group',function (e) {
			...
			$.post('<?php if (isset($edit_invoice)) { echo base_url()."Enterprise/invoice_transfer/".$code."/".$tid."/";}?>'+gid
			,function (data, status , xhr) {
				window.location = '<?php echo base_url()."Enterprise/invoice/".$mod_id."/".$code; ?>';
			}, 'text');
		});
		...
		$('#invoice_user_add').click(function (e) {
			...
			$.post('<?php if (isset($edit_invoice)) { echo base_url()."Enterprise/invoice_transfer_user/".$code."/".$tid; } ?>', {
				'cust_name' : name[0]
			}, function(data, status, xhr) {
     	...
     	$('#m_add_terms').click(function (e) {
     		e.preventDefault();
     		$.post('<?php echo base_url()."Enterprise/save_terms/Invoice/".$code; ?>', {
				'name' : $('#t_name').val()
			}, function(data, status, xhr) {
				var a = JSON.parse(data);
				terms_arr = [];
    	...
    	$('#formal_submit').click(function (e) {
    		...
    		$.post('<?php if (isset($edit_invoice)) { echo base_url()."Enterprise/invoice_update/formal/".$tid."/".$code; }else{ echo base_url()."Enterprise/invoice_save/formal/".$code; } ?>',{
    	...
    	$('#cust_draft').click(function (e) {
    		e.preventDefault();
    		$.post('<?php if (isset($edit_invoice)) { echo base_url()."Enterprise/invoice_update/draft/".$tid."/".$code; } else {echo base_url()."Enterprise/invoice_save/draft/".$code;} ?>',{
    		'customer' : $('#cust_name').val(),
			'txn_no' : $('#i_txn_no').val(),
			'txn_date' : $('#i_txn_date').val(),
			'product' : product_list,
    	...
    	$('#cust_print').click(function(e){
	    	...
	    	$.post('<?php echo base_url()."Enterprise/check_template/".$code."/".$mod_id; ?>',{
				...
				if (data == 'true') {
					window.location = "<?php if (isset($edit_invoice)) { echo base_url()."Enterprise/invoice_download/p/".$mod_id."/".$tid."/".$code;} ?>" ;
				}else{
					$('#sel_temp').modal('show');
				}
			}, "text");
	    ...
	    $('#cust_download').click(function(e){
	    	...
	    	$.post('<?php echo base_url()."Enterprise/check_template/".$code."/".$mod_id; ?>',{
				...
				if (data == 'true') {
					window.location = "<?php if (isset($edit_invoice)) { echo base_url()."Enterprise/invoice_download/d/".$mod_id."/".$tid."/".$code;} ?>" ;
				}else{
					$('#sel_temp').modal('show');
				}
			}, "text");
		...
		$('#cust_delete').click(function(e){
	    	e.preventDefault();
	    	window.location = "<?php if (isset($edit_invoice)) { echo base_url()."Enterprise/invoice_delete/".$mod_id."/".$tid."/".$code;} ?>" ;
	    });
		...
		$('#send_mail').click(function(e){
			e.preventDefault();
			$('.loader').show();
			$.post('<?php if (isset($edit_invoice)) echo base_url()."Enterprise/save_invoice_mail/".$mod_id."/".$tid."/".$code; ?>',
    	...
    	$('#email_submit').click(function (e) {
    		...
    		if($('.u_multiple')[0].files[0]) {
				...
				$.ajax({
					url: "<?php echo base_url().'Enterprise/invoice_email_doc_upload/'.$code.'/'; ?>"+ cust_name, // Url to which the request is send
    	...
    	function send_upload_mail() {
			...
			$.post('<?php echo base_url()."Enterprise/invoice_send_email/".$code; ?>', {
				'customer' : $('#cust_name').val(),
				'txn_no' : $('#i_txn_no').val(),
				'txn_date' : $('#i_txn_date').val(),
				'files' : repo,
		...
		function upload_files(pid){
    		...
    		if($('.proposal_doc')[0].files[0]) {
				...
				$.ajax({
					url: "<?php echo base_url().'Enterprise/invoice_doc_upload/'.$code.'/';?>"+pid+'/'+cust_name, // Url to which the request is send
					type: "POST",             // Type of request to be send, called as method
					data: datat, // Data sent to server, a set of key/value pairs (i.e. form fields and values)
					contentType: false,       // The content type used when sending data to the server.
					cache: false,             // To unable request pages to be cached
					...
					success: function(data)   // A function to be called if request succeeds
					{
						window.location = '<?php echo base_url()."Enterprise/invoice_add/".$mod_id."/".$code."/"; ?>'+pid;
					}
				});
			}else{
				window.location = '<?php echo base_url()."Enterprise/invoice_add/".$mod_id."/".$code."/"; ?>'+pid;
