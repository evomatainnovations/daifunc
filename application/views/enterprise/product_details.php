...
<main class="mdl-layout__content" style="z-index:3;">
	<div class="mdl-grid">
		<div class="mdl-cell mdl-cell--6-col">
			<h2><?php echo $p_name[0]->ip_product; ?></h2>
		</div>
		<div class="mdl-cell mdl-cell--6-col" style="text-align: right;">
            <?php
                if (isset($admin)) {
                    if ($admin == 'true') {
                        echo '<button class="mdl-button mdl-js-button mdl-js-ripple-effect mdl-button-raised" id="p_edit">Edit Product <i class="material-icons">edit</i></button>';
                    }
                }
            ?>
		</div>
	...
	<div class="mdl-grid">
		<div class="panel-group" id="accordion" style="width: 100%;margin-left: 10px;">
            ...
            <div class="panel panel-default amc" style="display: none;">
                <div class="panel-heading">
                    <h4 class="panel-title">
                        <a data-toggle="collapse" data-parent="#accordion" href="#amc">Maintenance</a>
                    </h4>
                </div>
            ...
            <div class="panel panel-default quotation" style="display: none;">
                <div class="panel-heading">
                    <h4 class="panel-title">
                        <a data-toggle="collapse" data-parent="#accordion" href="#quotation">Quotation</a>
                    </h4>
                </div>
                <div id="quotation" class="panel-collapse collapse">
                    <div class="panel-body"><table id="quotation_table" class="mdl-data-table mdl-js-data-table mdl-shadow--4dp" style="width: 100%;"></table></div>
                </div>
            </div>
...
var activity_arr = [],quotation_arr= [],amc_arr = [],purchase_arr =[], inventory_arr = [],invoice_arr = [],doc_arr = [],pic_arr = [],proposal_arr = [];
    <?php
            ...
            if (isset($doc)) {
                ...
                for ($i=0; $i <count($doc) ; $i++) { 
                    echo "doc_arr.push({'id' : ".$doc[$i]->ipp_id.", 'cid' : ".$doc[$i]->ipp_pid.", 'file' : '".$doc[$i]->ipp_file."', 'file_id' : '".$doc[$i]->ipp_timestamp."'});";
                }
            }
    ?>
    $(document).ready( function() {
        ...
        $('#p_edit').click(function(e){
            e.preventDefault();
            window.location = '<?php echo base_url()."Enterprise/product_edit/".$code."/".$p_id; ?>';
        });
    ...
    function display_details(){
        ...
        if (quotation_arr.length > 0) {
            out += '<thead><th class="mdl-data-table__cell--non-numeric">Title</th><th class="mdl-data-table__cell--non-numeric">Date</th><th class="mdl-data-table__cell--non-numeric">Status</th></thead><tbody>';
            for (var i = 0; i < quotation_arr.length; i++) {
                out +='<tr class="quotation_edit" id="'+quotation_arr[i].id+'">';
                out +='<td class="mdl-data-table__cell--non-numeric">'+quotation_arr[i].title+'</td><td class="mdl-data-table__cell--non-numeric">'+quotation_arr[i].date+'</td><td class="mdl-data-table__cell--non-numeric">'+quotation_arr[i].status+'</td>';
            }
            out+='</tbody>';
            $('#quotation_table').empty();
            $('#quotation_table').append(out);
        }else{
            out += '<thead><th class="mdl-data-table__cell--non-numeric" style="text-align:center;"><h3>No records found !!</h3></th></thead>';
            $('#quotation_table').empty();
            $('#quotation_table').append(out);
