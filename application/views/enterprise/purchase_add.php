	...
	<div class="mdl-grid">
					...
					<?php
						if (isset($invoice_property)) {
							for ($i = 0; $i < count($invoice_property); $i++) {
								if ($invoice_property[$i]->iexteprpt_status == 'true') {
									echo '<tr><td><input type="checkbox" id="'.$invoice_property[$i]->iexteprpt_id.'" checked></td><td style="padding: 10px;"> '.$invoice_property[$i]->iexteprpt_property_value.'</td></tr>';
								}else{
									echo '<tr><td><input type="checkbox" id="'.$invoice_property[$i]->iexteprpt_id.'"></td><td style="padding: 10px;"> '.$invoice_property[$i]->iexteprpt_property_value.'</td></tr>';
								}
		...
		<div class="mdl-cell mdl-cell--8-col" style="background-color: #fff;border-radius: 10px;box-shadow: 0px 4px 10px #ccc;">
			<div class="mdl-grid">
				<div class="mdl-cell mdl-cell--3-col">
					<div class="mdl-textfield mdl-js-textfield mdl-textfield--floating-label">
						<input type="text" id="i_txn_no" name="i_txn_no" class="mdl-textfield__input" value="<?php if(isset($edit_invoice)) { echo $edit_invoice[0]->iextep_txn_id; }else{ echo $invoice_doc_id; } ?>">
						<label class="mdl-textfield__label" for="i_txn_no">Enter Transaction Number</label>
					</div>
				</div>
				<div class="mdl-cell mdl-cell--3-col">
					<div class="mdl-textfield mdl-js-textfield mdl-textfield--floating-label">
						<input type="text" data-type="date" id="i_txn_date" class="mdl-textfield__input" value="<?php if(isset($edit_invoice)) { echo $edit_invoice[0]->iextep_txn_date; }else{echo date('Y-m-d');} ?>">
				...
				<?php 
					if(isset($edit_invoice)) {
						...
						if ($edit_invoice[0]->iextep_status == "unpaid") {
							echo '<option value="unpaid" selected>Unpaid</option>';
						} else {
							echo '<option value="unpaid">Unpaid</option>';
						}

						if ($edit_invoice[0]->iextep_status == "paid") {
							echo '<option value="paid" selected>Paid</option>';
						} else {
							echo '<option value="paid">Paid</option>';
						}

						if ($edit_invoice[0]->iextep_status == "negotiate") {
							echo '<option value="negotiate" selected>Negotiate</option>';
						} else {
							echo '<option value="negotiate">Negotiate</option>';
						}
				...
				<div class="mdl-cell mdl-cell--3-col">
					<div class="mdl-textfield mdl-js-textfield mdl-textfield--floating-label">
						<input type="text" id="i_wrnty" class="mdl-textfield__input" value="<?php if(isset($edit_invoice)) { echo $edit_invoice[0]->iextep_warranty; } ?>">
			...
			<div class="mdl-grid" style="text-align: right; display: inline-flex;">
				<?php
					if(isset($edit_invoice)) {
						echo '<button type="button" class="mdl-button mdl-button--colored" data-toggle="collapse" style="margin-top : 10px" data-target="#ownership">OWNERSHIP AND GROUP</button>';
	...
	<div class="mdl-grid">
		<div class="mdl-tabs mdl-js-tabs mdl-js-ripple-effect">
            <div class="mdl-tabs__tab-bar" style="width:100%">
	            <a href="#p_formal" class="mdl-tabs__tab is-active" id="formal" style="color:black">Formal Purchase</a>
            </div>
            				...
            				<div class="mdl-grid">
								...
								<div class="mdl-cel mdl-cell--2-col">
									<div class="mdl-textfield mdl-js-textfield mdl-textfield--floating-label">
										...
										<label class="mdl-textfield__label" for="prod_sn">Enter S/N</label>
									</div>
                	...
                	<div class="mdl-grid">
        				...
        				<h3>Add Note</h3>
        					<div class="mdl-cell--10-col" style="padding-top: 20px;">
        						<div contenteditable="true" id="pro_formal_note" style="font-size: 16px;"><?php if (isset($edit_invoice)) { echo $edit_invoice[0]->iextep_note;}?></div>
        					</div>
                		</div>
                		<div class="mdl-cell mdl-cell--6-col">
                		<h3>Tags</h3>
...
<div class="modal fade" id="sel_temp" role="dialog">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body" style="text-align: center;">
				<?php echo '<a href="'.base_url().'Enterprise/module_setting/Purchase/'.$code.'"><button class="mdl-button mdl-js-button mdl-button--raise mdl-button--colored"> Click to select template</button><a>'; ?>
			</div>
		</div>
	</div>
</div>
	...
	var user_data = [];
    	<?php
    		...
    		if (isset($edit_invoice)) {
    			for ($i=0; $i <count($edit_invoice) ; $i++) { 
    				echo "product_list.push({'id' : ".$i.",'product' : '".$edit_invoice[$i]->ip_product."', 'rate' : '".$edit_invoice[$i]->iexteppd_rate."', 'qty' : '".$edit_invoice[$i]->iexteppd_qty."', 'disc' : '".$edit_invoice[$i]->iexteppd_discount."', 'tax_id' : '".$edit_invoice[$i]->iexteppd_tax."' , 'sn' : '".$edit_invoice[$i]->iexteppd_serial_number."', 'alias' : '".$edit_invoice[$i]->iexteppd_alias."' });";
    			}

    			if (count($invoice_property) > 0) {
    				for ($i=0; $i <count($invoice_property) ; $i++) { 
    					echo "property_arr.push({'id' : ".$invoice_property[$i]->iexteprpt_id." ,'value' : '".$invoice_property[$i]->iexteprpt_property_value."','status': '".$invoice_property[$i]->iexteprpt_status."' });";
    				}
    			}
    		}
    		...
    		if (isset($p_terms)) {
    			if (count($p_terms) > 0) {
    				for ($i=0; $i <count($p_terms) ; $i++) {
    					...
    					if ($flg == 'true') {
    						echo "terms_arr.push({'id' : ".$p_terms[$i]->iextdt_id.", 'terms' : '".$p_terms[$i]->iextdt_term."', 'status' : '".$p_terms[$i]->iexteprtm_status."'});";
    					}else{
	 ...
	 $(document).ready( function() {
		...
		$('.pay_save').click(function (e) {
			...
			$.post('<?php if (isset($edit_invoice)) { echo base_url()."Enterprise/pay_save/".$mod_id."/".$code."/".$tid."/";}?>'+pay_edit,{
				...
				window.location = '<?php if (isset($edit_invoice)) echo base_url()."Enterprise/purchase_add/".$code."/".$mod_id."/".$tid;?>';
			}, "text");
		});
		...
		$('.pay_edit').click(function (e) {
     	...
     	$('#m_add_terms').click(function (e) {
     		e.preventDefault();
     		$.post('<?php echo base_url()."Enterprise/save_terms/Purchase/".$code; ?>', {
				'name' : $('#t_name').val()
			}, function(data, status, xhr) {
				var a = JSON.parse(data);
				terms_arr = [];
    	...
    	$('.pro_to_self').click(function (e) {
        	e.preventDefault();
        	$.post('<?php if (isset($edit_invoice)) { echo base_url()."Enterprise/purchase_transfer/".$code."/".$tid."/0";}?>'
			, function(data, status, xhr) {
				window.location = '<?php echo base_url()."Enterprise/purchase/".$mod_id."/".$code; ?>';
			}, "text");
        });
		...
		$('#grp_body').on('click','.transfer_to_group',function (e) {
			...
			$.post('<?php if (isset($edit_invoice)) { echo base_url()."Enterprise/purchase_transfer/".$code."/".$tid."/";}?>'+gid
			,function (data, status , xhr) {
				window.location = '<?php echo base_url()."Enterprise/purchase/".$mod_id."/".$code; ?>';
			}, 'text');
		});
		...
		$('#invoice_user_add').click(function (e) {
			...
			$.post('<?php if (isset($edit_invoice)) { echo base_url()."Enterprise/purchase_transfer_user/".$code."/".$tid; } ?>', {
				'cust_name' : name[0]
			}, function(data, status, xhr) {
    	...
    	$('#formal_submit').click(function (e) {
    		...
    		$.post('<?php if (isset($edit_invoice)) { echo base_url()."Enterprise/purchase_update/formal/".$code."/".$tid; }else{ echo base_url()."Enterprise/save_purchase/formal/".$code; } ?>',{
    	...
    	$('#cust_draft').click(function (e) {
    		...
    		$.post('<?php if (isset($edit_invoice)) { echo base_url()."Enterprise/purchase_update/draft/".$tid; } else {echo base_url()."Enterprise/save_purchase/draft/".$code;} ?>',{
	    		'customer' : $('#cust_name').val(),
				'txn_no' : $('#i_txn_no').val(),
				'txn_date' : $('#i_txn_date').val(),
				'product' : product_list,
    	...
    	$('#cust_print').click(function(e){
	    	...
	    	$.post('<?php echo base_url()."Enterprise/check_template/".$code."/".$mod_id; ?>',{
				...
				if (data == 'true') {
					window.location = "<?php if (isset($edit_invoice)) { echo base_url()."Enterprise/purchase_download/p/".$mod_id."/".$tid."/".$code;} ?>";
				}else{
					$('#sel_temp').modal('show');
				}
			}, "text");
    	...
    	$('#cust_download').click(function(e){
	    	...
	    	$.post('<?php echo base_url()."Enterprise/check_template/".$code."/".$mod_id; ?>',{
				...
				if (data == 'true') {
					window.location = "<?php if (isset($edit_invoice)) { echo base_url()."Enterprise/purchase_download/d/".$mod_id."/".$tid."/".$code;} ?>" ;
				}else{
					$('#sel_temp').modal('show');
				}
			}, "text");
	 ...
	 //       	email_append();
		...
		$('#cust_delete').click(function(e){
	    	e.preventDefault();
	    	window.location = "<?php if (isset($edit_invoice)) { echo base_url()."Enterprise/purchase_delete/".$code."/".$mod_id."/".$tid."/".$code;} ?>" ;
	    });
		...
		$('#send_mail').click(function(e){
			e.preventDefault();
			$('.loader').show();
			$.post('<?php if (isset($edit_invoice)) echo base_url()."Enterprise/save_purchase_mail/".$mod_id."/".$tid."/".$code; ?>',
		...
		function getprice(product) {
			$.post('<?php echo base_url()."Enterprise/purchase_get_price/".$code; ?>', {
				"product" : product
			}, function(data, status, xhr) {
				if (data == 'false') {
					$('#prod_rate').focus();
		...
		function upload_files(pid,cid){
    		...
    		if($('.proposal_doc')[0].files[0]) {
				...
				$.ajax({
					url: "<?php echo base_url().'Enterprise/purchase_doc_upload/'.$code."/";?>"+pid+'/'+cid, // Url to which the request is send
					...
					{
						window.location = "<?php echo base_url().'Enterprise/purchase_add/'.$code.'/'.$mod_id.'/' ?>"+pid;
					}
				});
			}else{
				window.location = "<?php echo base_url().'Enterprise/purchase_add/'.$code.'/'.$mod_id.'/' ?>"+pid;
