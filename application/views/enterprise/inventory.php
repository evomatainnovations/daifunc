...
<main class="mdl-layout__content">
	...
	<div class="mdl-grid" style="margin-top: 0px;">	
		<!-- <div class="mdl-cell mdl-cell--3-col"> -->
			<a href="<?php echo base_url().'Enterprise/inventory_add/inward/'.$code.'/'.$mod_id; ?>">
				<button class="mdl-button" style="margin-left: 10px;"><i class="material-icons">arrow_downward</i> Add Inward</button>
			</a>
		<!-- </div> -->
		<!-- <div class="mdl-cell mdl-cell--3-col"> -->
			<a href="<?php echo base_url().'Enterprise/inventory_add/outward/'.$code.'/'.$mod_id; ?>">
				<button class="mdl-button" style="margin-left: 10px;"><i class="material-icons">arrow_upward</i> Add Outward</button>
			</a>
		<!-- </div>
		<div class="mdl-cell mdl-cell--2-col"> -->
			<a href="<?php echo base_url().'Enterprise/inventory_spare/'.$code.'/'.$mod_id; ?>">
				<button class="mdl-button" style="margin-left: 10px;"><i class="material-icons">swap_horiz</i> Spares Details</button>
			</a>
		<!-- </div>
		<div class="mdl-cell mdl-cell--2-col"> -->
			<a href="<?php echo base_url().'Enterprise/inventory_defective/'.$code.'/'.$mod_id; ?>">
				<button class="mdl-button" style="margin-left: 10px;"><i class="material-icons">build</i> Defective Details</button>
			</a>
		<!-- </div>
		<div class="mdl-cell mdl-cell--2-col"> -->
			<a href="<?php echo base_url().'Enterprise/inventory_status/'.$code; ?>">
				<button class="mdl-button" style="margin-left: 10px;"><i class="material-icons">trending_up</i> Stock Status</button>
			</a>
		<!-- </div> -->
	</div>
	<div class="mdl-grid">
			...
			<tbody id="details">
				<?php
					for ($i=0; $i < count($inventory) ; $i++) { 
						...
						// else if($inventory[$i]->iextei_type == "spare"){
						// 	echo '<tr style="color: #fff; background-color: rgb(42, 136, 231);font-weight: bold;" class="tbl_view_spare" id="'.$inventory[$i]->iextei_id.'">';
						// 	echo '<td class="mdl-data-table__cell--non-numeric">'.$inventory[$i]->iextei_txn_id.'</td>';
						// 	echo '<td class="mdl-data-table__cell--non-numeric">'.$inventory[$i]->iextei_txn_date.'</td>';
						// 	echo '<td class="mdl-data-table__cell--non-numeric">'.$inventory[$i]->ic_name.'</td>';
						// 	echo '</tr>';
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#details').on('click', '.tbl_view_inward', (function(e) {
			...
			window.location = "<?php echo base_url().'Enterprise/inventory_edit/inward/'.$code.'/'.$mod_id.'/'; ?>" + tid;
		}));

		$('#details').on('click', '.tbl_view_spare', (function(e) {
			e.preventDefault();
			var tid = $(this).prop('id');
			window.location = "<?php echo base_url().'Enterprise/spare_edit/'.$code.'/'.$mod_id.'/'; ?>" + tid;
		}));

		$('#invoice_setting').click(function (e){
			e.preventDefault();
			window.location = "<?php echo base_url().'Enterprise/module_setting/Inventory/'.$code; ?>";
		});

		$('#details').on('click', '.tbl_view_outward', (function(e) {
			e.preventDefault();
			var tid = $(this).prop('id');
			window.location = "<?php echo base_url().'Enterprise/inventory_edit/outward/'.$code.'/'.$mod_id.'/'; ?>" + tid;
		}));

		$('#fixed-header-drawer-exp').change(function(e) {
			e.preventDefault();

			$.post('<?php echo base_url()."Enterprise/inventory_search/".$code; ?>', {
				'search' : $(this).val()
				...
				for (var i = 0; i < abc.inventory.length; i++) {
					...
					// if(abc.inventory[i].iextei_type == "spare"){
					// 	out+='<tr style="color: #fff; background-color: rgb(42, 136, 231);font-weight: bold;" class="tbl_view_spare" id="' + abc.inventory[i].iextei_id + '">';
					// 	out+='<td class="mdl-data-table__cell--non-numeric">' + abc.inventory[i].iextei_txn_id + '</td>';
					// 	out+='<td class="mdl-data-table__cell--non-numeric">' + abc.inventory[i].iextei_txn_date + '</td>';
					// 	out+='<td class="mdl-data-table__cell--non-numeric">' + abc.inventory[i].ic_name + '</td>';
					// 	out+='</tr>';
		...
		$('#check').click(function(e) {
			e.preventDefault();
			$.post('<?php echo base_url()."Enterprise/inventory_filter/".$code; ?>', {
				'from' : $('#fr_date').val(),
				'to': $('#to_date').val(),
				'in_type'	 : $('#in_type').val(),
