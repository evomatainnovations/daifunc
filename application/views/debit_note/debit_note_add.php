...
<div class="modal fade" id="sel_temp" role="dialog">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body" style="text-align: center;">
				<?php echo '<a href="'.base_url().'Enterprise/module_setting/Debit_note/'.$code.'"><button class="mdl-button mdl-js-button mdl-button--raise mdl-button--colored"> Click to select template</button><a>'; ?>
			</div>
		</div>
	</div>
</div>
	...
	?>
	 $(document).ready( function() {
    	...
    	$('.doc_file').click(function (e) {
     		e.preventDefault();
     		var id = $(this).prop('id');
			window.location = '<?php echo base_url()."Debit_note/upload_doc_download/".$code."/";?>'+id;
     	});

    	$('.pro_to_self').click(function (e) {
        	e.preventDefault();
        	$.post('<?php if (isset($edit_invoice)) { echo base_url()."Debit_note/invoice_transfer/".$code."/".$tid."/0";}?>'
			, function(data, status, xhr) {
				window.location = '<?php echo base_url()."Debit_note/debit_note/".$mod_id."/".$code; ?>';
			}, "text");
        });
    	var pay_edit = 0;
		...
		$('.pay_save').click(function (e) {
			...
			$.post('<?php if (isset($edit_invoice)) { echo base_url()."Enterprise/pay_save/".$mod_id."/".$code."/".$tid."/";}?>'+pay_edit,{
				...
				window.location = '<?php if (isset($edit_invoice)) echo base_url()."Debit_note/invoice_add/".$mod_id."/".$code."/".$tid; ?>';
			}, "text");
		});
		...
		$('#grp_body').on('click','.transfer_to_group',function (e) {
			...
			$.post('<?php if (isset($edit_invoice)) { echo base_url()."Debit_note/invoice_transfer/".$code."/".$tid."/";}?>'+gid
			,function (data, status , xhr) {
				window.location = '<?php echo base_url()."Debit_note/debit_note/".$mod_id."/".$code; ?>';
			}, 'text');
		});
		...
		$('#invoice_user_add').click(function (e) {
			...
			$.post('<?php if (isset($edit_invoice)) { echo base_url()."Debit_note/invoice_transfer_user/".$code."/".$tid; } ?>', {
				'cust_name' : name[0]
			}, function(data, status, xhr) {
     	...
     	$('#pro_title').change(function (e) {
     		...
     		$.post('<?php echo base_url()."Debit_note/get_email_body/".$code."/";?>'+e_title
			, function(data, status, xhr) {
     	...
     	$('#add_property').click(function (e) {
     		...
     		if (cust == '') {
	    		...
	    		snackbarContainer.MaterialSnackbar.showSnackbar(data);
     			...
     			$.post('<?php echo base_url()."Debit_note/cust_add_property/".$code; ?>', {
					'c_name' : cust
				}, function(data, status, xhr) {
     	...
     	$('#m_add_terms').click(function (e) {
     		e.preventDefault();
     		$.post('<?php echo base_url()."Debit_note/save_terms/Debit_note/".$code; ?>', {
				'name' : $('#t_name').val()
			}, function(data, status, xhr) {
				var a = JSON.parse(data);
				terms_arr = [];
    	...
    	$('#formal_submit').click(function (e) {
    		...
    		$.post('<?php if (isset($edit_invoice)) { echo base_url()."Debit_note/invoice_update/formal/".$tid."/".$code; }else{ echo base_url()."Debit_note/invoice_save/formal/".$code; } ?>',{
    	...
    	$('#cust_draft').click(function (e) {
    		...
    		$.post('<?php if (isset($edit_invoice)) { echo base_url()."Debit_note/invoice_update/draft/".$tid."/".$code; } else {echo base_url()."Debit_note/invoice_save/draft/".$code;} ?>',{
    		'customer' : $('#cust_name').val(),
			'txn_no' : $('#i_txn_no').val(),
			'txn_date' : $('#i_txn_date').val(),
			'product' : product_list,
    	...
    	$('#cust_print').click(function(e){
	    	...
	    	$.post('<?php echo base_url()."Enterprise/check_template/".$code."/".$mod_id; ?>',{
				...
				if (data == 'true') {
					window.location = "<?php if (isset($edit_invoice)) { echo base_url()."Debit_note/invoice_download/p/".$mod_id."/".$tid."/".$code;} ?>" ;
				}else{
					$('#sel_temp').modal('show');
				}
			}, "text");
	    ...
	    $('#cust_download').click(function(e){
	    	...
	    	$.post('<?php echo base_url()."Enterprise/check_template/".$code."/".$mod_id; ?>',{
				...
				if (data == 'true') {
					window.location = "<?php if (isset($edit_invoice)) { echo base_url()."Debit_note/invoice_download/d/".$mod_id."/".$tid."/".$code;} ?>" ;
				}else{
					$('#sel_temp').modal('show');
				}
			}, "text");
		...
		$('#cust_delete').click(function(e){
	    	e.preventDefault();
	    	window.location = "<?php if (isset($edit_invoice)) { echo base_url()."Debit_note/invoice_delete/".$mod_id."/".$tid."/".$code;} ?>" ;
	    });
		...
		$('#send_mail').click(function(e){
			e.preventDefault();
			$('.loader').show();
			$.post('<?php if (isset($edit_invoice)) echo base_url()."Debit_note/save_invoice_mail/".$mod_id."/".$tid."/".$code; ?>',
    	...
    	$('#email_submit').click(function (e) {
    		...
    		if($('.u_multiple')[0].files[0]) {
				...
				$.ajax({
					url: "<?php echo base_url().'Debit_note/invoice_email_doc_upload/'.$code.'/'; ?>"+ cust_name, // Url to which the request is send
    	...
    	function send_upload_mail() {
			...
			$.post('<?php echo base_url()."Debit_note/invoice_send_email/".$code; ?>', {
				'customer' : $('#cust_name').val(),
				'txn_no' : $('#i_txn_no').val(),
				'txn_date' : $('#i_txn_date').val(),
				'files' : repo,
		...
		function upload_files(pid,cid){
    		...
    		if($('.proposal_doc')[0].files[0]) {
				...
				$.ajax({
					url: "<?php echo base_url().'Debit_note/invoice_doc_upload/'.$code.'/';?>"+pid+'/'+cust_name, // Url to which the request is send
					type: "POST",             // Type of request to be send, called as method
					data: datat, // Data sent to server, a set of key/value pairs (i.e. form fields and values)
					contentType: false,       // The content type used when sending data to the server.
					cache: false,             // To unable request pages to be cached
					...
					success: function(data)   // A function to be called if request succeeds
					{
						window.location = '<?php echo base_url()."Debit_note/invoice_add/".$mod_id."/".$code."/"; ?>'+pid;
					}
				});
			}else{
				window.location = '<?php echo base_url()."Debit_note/invoice_add/".$mod_id."/".$code."/"; ?>'+pid;
    	...
    	function get_details(customer) {
			$.post('<?php echo base_url()."Debit_note/cust_details/".$code."/"; ?>', {
				'c' : customer,
				}, function(data, status, xhr) {
				var a = JSON.parse(data);
				var out = '';
		...
		function get_prod_details(product_name) {
			$.post('<?php echo base_url()."Debit_note/invoice_product_rate/".$code."/"; ?>',{
				'pname' : product_name
			}, function(data, status, xhr) {
				if (data == 'false') {
					$('#prod_rate').focus();
