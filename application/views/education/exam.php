<main class="mdl-layout__content">
	...
	<a href="<?php echo base_url().'education/exam_add'; ?>">
		...
		function getdetails(dt) {
			$.post('<?php echo base_url()."education/get_exams/"; ?>' + dt,
				...
				for (var i = 0; i < abc.length; i++) {
					var fdt = new Date(abc[i].iextes_from_date);
					var tdt = new Date(abc[i].iextes_to_date);
					file += '<div class="mdl-cell mdl-cell--2-col" style="text-align:left!important;"><a href="<?php echo base_url()."education/edit_exam/"; ?>' + abc[i].iextes_id + '"><div class="mdl-card mdl-shadow--4dp"><div class="mdl-card__title mdl-card--expan" style="height:150px;"><h class="mdl-card__title-text"style="text-align:left!important;font-size:25px;">Timings:<br>' + fdt.getHours().toString() + ':' + fdt.getMinutes().toString() + '-<br>' + tdt.getHours().toString() + ':' + tdt.getMinutes().toString() + '</h></div><div class="mdl-card__supporting-text" style="text-align:left;">' + abc[i].iexts_name + '<br><br>';
					if (abc[i].iextes_type == "preliem") {
						file += abc[i].iextp_preliem_name
					} else if(abc[i].iextes_type == "chapter") {
						file += abc[i].iextc_name
					}
