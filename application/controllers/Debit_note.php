...
class Debit_note extends CI_Controller {
	...
	public function Debit_note($module_id,$code) {
		...
		if($sess_data['session'] == 'true') {
			...
			if ($gid == 0) {
				$query = $this->db->query("SELECT * FROM i_ext_et_invoice AS a LEFT JOIN i_customers AS b ON a.iextein_customer_id=b.ic_id WHERE a.iextein_owner = '$oid' AND a.iextein_gid = '0' AND a.iextein_txn_type = 'debit' ORDER BY a.iextein_id DESC");
				$result = $query->result();
				$data['invoice'] = $result;
			}else{
				...
				if (count($result) > 0 || $uid == $oid) {
					$query = $this->db->query("SELECT * FROM i_ext_et_invoice AS a LEFT JOIN i_customers AS b ON a.iextein_customer_id=b.ic_id WHERE a.iextein_owner = '$oid' AND a.iextein_gid = '$gid' AND a.iextein_txn_type = 'debit' ORDER BY a.iextein_id DESC");
					$result = $query->result();
					$data['invoice'] = $result;
				}else{
					$query = $this->db->query("SELECT * FROM i_ext_et_invoice AS a LEFT JOIN i_customers AS b ON a.iextein_customer_id=b.ic_id WHERE a.iextein_gid = '$gid' AND a.iextein_created_by = '$uid' UNION SELECT * FROM i_ext_et_invoice AS c LEFT JOIN i_customers AS d ON c.iextein_customer_id=d.ic_id WHERE c.iextein_gid = '$gid' AND a.iextein_txn_type = 'debit' AND c.iextein_id IN(SELECT iexteim_pid FROM i_ext_et_invoice_mutual WHERE iexteim_uid = '$uid' AND iexteim_oid = '$oid')");
					$result = $query->result();
					$data['invoice'] = $result;
				}
			}

			$query = $this->db->query("SELECT iextein_status FROM i_ext_et_invoice WHERE iextein_status != '' AND iextein_txn_type = 'debit' GROUP BY iextein_status");
			$result = $query->result();
			$data['status'] = $result;
			...
			if ($alias == '') {
				$ert['title'] = 'Debit Note';
			}else{
				$ert['title'] = $alias;
			}
			$ert['search'] = "true";
			$this->load->view('navbar', $ert);
			$this->load->view('Debit_note/Debit_note', $data);
	...
	public function invoice_search($code) {
		...
		if($sess_data['session'] == 'true') {
			...
			$query = $this->db->query("SELECT * FROM i_ext_et_invoice AS a LEFT JOIN i_customers AS b ON a.iextein_customer_id=b.ic_id WHERE a.iextein_owner = '$oid' AND a.iextein_txn_type = 'debit' AND a.iextein_txn_id LIKE '%".$search."%' OR b.ic_name LIKE '%".$search."%' ORDER BY a.iextein_id DESC");
			$result = $query->result();
			$data['invoice'] = $result;
	...
	public function invoice_filter($code) {
		...
		if($sess_data['session'] == 'true') {
			...
			$this->db->where('i_ext_et_invoice.iextein_txn_type', 'debit');
	...
	public function invoice_download($flg,$mod_id,$invoiceid,$code){
		...
		if($sess_data['session'] == 'true') {
		    ...
		    // redirect(base_url().'Purchase_order/invoice_print/'.$code.'/'.$mod_id.'/'.$invoiceid);
	 		$page = file_get_contents(base_url().'Debit_note/invoice_print/'.$code.'/'.$mod_id.'/'.$invoiceid, false, $context);
		    session_start();

		    $path = $this->config->item('document_rt').'assets/data/'.$oid.'/Debit_note/';
		    ...
		    if($flg == 'd'){
		    	...
		    	redirect(base_url().'assets/data/'.$oid.'/Debit_note/'.$invoicefile);
		    }
		}else{
			redirect(base_url().'Account/login');
		}
	...
	public function invoice_transfer($code,$pid,$gid){
		...
		if($sess_data['session'] == 'true') {
			...
			$this->db->WHERE(array('iextein_id' => $pid, 'iextein_owner' => $oid , 'iextein_txn_type' => 'debit'));
	...
	public function invoice_transfer_user($code,$inid){
		...
		if($sess_data['session'] == 'true') {
			...
			if (count($result) > 0) {
				...
				$this->db->where(array('iextein_id' => $inid, 'iextein_owner' => $oid , 'iextein_txn_type' => 'debit'));
	...
	public function invoice_add($module_id,$code,$tid=null) {
		...
		if($sess_data['session'] == 'true') {
			...
			for ($i=0; $i <count($result) ; $i++) {
				if ($result[$i]->iumdi_variable == 'false') {
					...
					if ($result[$i]->iumdi_doc_syntax == 'acc_yr') {
						...
						$query = $this->db->query("SELECT * FROM i_ext_et_invoice WHERE iextein_owner = '$oid' AND iextein_txn_type = 'debit' ");
			...
			if ($tid != null && $tid != 'null') {
				$query = $this->db->query("SELECT * FROM i_ext_et_invoice as a WHERE a.iextein_id ='$tid' AND a.iextein_owner = '$oid' AND a.iextein_txn_type = 'debit' ");
				$result = $query->result();
				...
				if ($in_amount == 0) {
					$this->db->WHERE(array('iextein_id'=>$tid,'iextein_owner'=>$oid , 'iextein_txn_type' => 'debit'));
				...
				$query = $this->db->query("SELECT * FROM i_ext_et_invoice as a left JOIN i_ext_et_invoice_product_details as b on a.iextein_id=b.iexteinpd_d_id LEFT join i_product as c on b.iexteinpd_product_id=c.ip_id WHERE a.iextein_id ='$tid' AND a.iextein_owner = '$oid' AND a.iextein_txn_type = 'debit' ");
				$result = $query->result();	
				$data['edit_invoice'] = $result;
				$data['edit_type'] = $result[0]->iextein_type;
				$data['invoice_gid'] = $result[0]->iextein_gid;
				if ($alias == '') {
				$ert['title'] = "Debit Note Edit";
				}else{
					$ert['title'] = $alias." Edit";
				}
			}else{
				$query = $this->db->query("SELECT * FROM i_ext_et_document_terms WHERE iextdt_owner = '$oid' AND iextdt_document='debit_note'");
				$result = $query->result();
				$data['term_doc'] = $result;
				$pay=0;
				if ($alias == '') {
					$ert['title'] = "Debit Note Add";
				}else{
					$ert['title'] = $alias." Add";
				}
			}
			...
			$this->load->view('Debit_note/debit_note_add', $data);
			$this->load->view('home/search_modal');
		} else {
			redirect(base_url().'Account/login');
		}
	...
	public function invoice_print($code,$mod_id, $invoice_id) {
		...
		if($sess_data['session'] == 'true') {
			...
			$query = $this->db->query("SELECT * FROM i_ext_et_document_terms as a LEFT JOIN i_ext_et_invoice_terms as b on a.iextdt_id=b.iexteintm_term_id WHERE iextdt_document='debit_note' AND iextdt_owner='$oid' AND iexteintm_inid= '$invoice_id' AND iexteintm_status = 'true' ");
			$result = $query->result();
			$data['terms'] = $result;
	...
	public function invoice_update($type,$tid,$code){
		...
		if($sess_data['session'] == 'true') {
			...
			$this->db->WHERE(array('iet_type_id'=>$tid,'iet_owner' => $oid, 'iet_type' => 'debit_note', 'iet_m_id' => $module_id));
	...
	public function invoice_delete($mod_id,$prid,$code) {
		...
		if($sess_data['session'] == 'true') {
			...
			redirect(base_url().'Debit_note/Debit_note/'.$mod_id.'/'.$code);
		} else {
			redirect(base_url().'Account/login');
		}
	}
	...
	public function send_invoice_email($uid, $mod_id, $invoiceid,$code) {
		...
		if($sess_data['session'] == 'true') {
			...
			if (count($result3)>0) {
				...
				$subject = $result[0]->iud_company.' - debit Note - '.$result2[0]->iextein_txn_id;
